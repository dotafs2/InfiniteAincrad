...
const COTTAGE_REBAKE_CANDIDATE_PATH := "res://assets/floor1/meshy_houses_lod/candidates/01_hearth_cottage_uv_rebaked_60000_color2k.glb"
...
var _north_grove_probe_dir := ""
var _north_grove_trial := true
var _cottage_rebake_probe_dir := ""
var _cottage_rebake_trial := false
var _tour_output := ""
...
var _north_grove_trees: Array[Node3D] = []
var _cottage_ab_house: Node3D
var _cottage_rebake_visual: Node3D
...
func _ready() -> void:
	for arg in OS.get_cmdline_user_args():
		...
		elif arg.begins_with("--north-grove-probe-dir="):
			_north_grove_probe_dir = arg.trim_prefix("--north-grove-probe-dir=")
		elif arg.begins_with("--north-grove-trial="):
			_north_grove_trial = arg.trim_prefix("--north-grove-trial=") in ["on", "true", "1"]
		elif arg.begins_with("--cottage-rebake-probe-dir="):
			_cottage_rebake_probe_dir = arg.trim_prefix("--cottage-rebake-probe-dir=")
		elif arg.begins_with("--cottage-rebake-trial="):
			_cottage_rebake_trial = arg.trim_prefix("--cottage-rebake-trial=") in ["on", "true", "1"]
	...
	if not review_mode:
		...
		elif not _north_grove_probe_dir.is_empty():
			_make_review_camera()
			_capture_north_grove_probe.call_deferred()
		elif not _cottage_rebake_probe_dir.is_empty():
			_make_review_camera()
			_capture_cottage_rebake_probe.call_deferred()
...
func _finish_tour() -> void:
	if _tour_version in ["v2", "v3"]: _stage_tour_v2_door(60.0)
	var rows := []
	var elapsed := 0.0
	for key: Dictionary in _tour_keyframes:
	...
	var report := {"suite":"floor1_expanded_world_continuous_tour", "scene":"res://scenes/floor1_expanded_world.tscn",
		...
		"duration_s":60 if _tour_version in ["v2", "v3"] else 30,
		...
		"staged_real_bakery_door_open_close_s":[17, 30] if _tour_version in ["v2", "v3"] else [],
		"temporary_real_door_actions":2 if _tour_version in ["v2", "v3"] else 0,
		"approved_nw_native_t2_pocket_total_trees":(1 if _native_forest_oak_trial != null else 0) + _north_grove_trees.size(),
		"approved_nw_native_t2_grove_additional_trees":_north_grove_trees.size(),
		"selected_exterior_texture_tier":exterior_texture_tier,
		"country_grass_texture_mix":meadow_texture_mix,
...
func _place_houses() -> void:
	...
	# The approved 60K/2K rebake is an isolated *one-house* art candidate.
	# The original 32-house 2K LOD set remains untouched and default visible.
	for exterior in exterior_houses:
		if (String(exterior.get("house_id")) == "01_hearth_cottage" and
			String(exterior.get_meta("district", "")) == "MarketFrontage" and
			exterior.position.x < 0.0):
			_cottage_ab_house = exterior
			break
	if _cottage_rebake_trial and _cottage_ab_house != null:
		_install_one_cottage_rebake_candidate(_cottage_ab_house)
	for house in interactive_houses:
...
func _install_one_cottage_rebake_candidate(house: Node3D) -> void:
	var packed := load(COTTAGE_REBAKE_CANDIDATE_PATH) as PackedScene
	if packed == null:
		push_error("Isolated 60K/2K cottage rebake failed to import")
		return
	var holder := Node3D.new()
	holder.name = "OneWestMarketCottage60k2kRebakeCandidate"
	house.add_child(holder)
	var visual := Node3D.new()
	visual.name = "CandidateNormalized10mVisual"
	holder.add_child(visual)
	visual.add_child(packed.instantiate())
	# Measure actual imported GLB geometry, then apply the same artist-chosen
	# 10m height/floor/XZ-centre as the accepted 38K Meshy cottage LOD0.
	var raw_low := Vector3(INF, INF, INF)
	var raw_high := Vector3(-INF, -INF, -INF)
	var actual_tris := 0
	var actual_surfaces := 0
	for mesh: MeshInstance3D in visual.find_children("*", "MeshInstance3D", true, false):
		...
		for xx in [box.position.x, box.end.x]:
			for yy in [box.position.y, box.end.y]:
				for zz in [box.position.z, box.end.z]:
					var point := visual.global_transform.affine_inverse() * (
						mesh.global_transform * Vector3(xx, yy, zz))
		...
		for surface in range(mesh.mesh.get_surface_count()):
			...
			actual_tris += int(indices.size() / 3) if not indices.is_empty() else int(vertices.size() / 3)
	if raw_high.y <= raw_low.y or actual_tris == 0:
		push_error("Candidate cottage has invalid imported bounds/triangles")
		return
	var scale_value := 10.0 / (raw_high.y - raw_low.y)
	visual.scale = Vector3.ONE * scale_value
	visual.position = Vector3(-0.5 * (raw_low.x + raw_high.x) * scale_value,
		-raw_low.y * scale_value, -0.5 * (raw_low.z + raw_high.z) * scale_value)
	var baseline := house.get_node_or_null("MeshyHouseGeometry") as Node3D
	if baseline != null: baseline.visible = false
	holder.set_meta("asset_path", COTTAGE_REBAKE_CANDIDATE_PATH)
	holder.set_meta("raw_bounds_low", raw_low)
	holder.set_meta("raw_bounds_high", raw_high)
	holder.set_meta("actual_imported_triangles", actual_tris)
	holder.set_meta("actual_imported_surfaces", actual_surfaces)
	holder.set_meta("display_height_m", 10.0)
	holder.set_meta("hidden_default_lod_baseline_still_loaded", true)
	_cottage_rebake_visual = holder
...
func _northwest_ground_sector() -> void:
	...
	if _north_grove_trial:
		_place_north_grove_trial(x_bands, z_bands)
...
func _place_north_grove_trial(x_bands: Array, z_bands: Array) -> void:
	# Nine additional native T2 oaks make a root-reviewed ten-tree pocket with
	# the one approved front-edge focal oak. This 35x40m group is default-on;
	# --north-grove-trial=off keeps the proven single-tree v2 fallback. Trees use the
	# exact same imported PackedScene/mesh/material, varied display heights,
	# actual low-poly triangle-floor pivots and trunk-only collision.
	var packed := load(NATIVE_FOREST_OAK_TRIAL_PATH) as PackedScene
	if packed == null:
		push_error("NW grove T2 oak trial failed to import")
		return
	var sites := [
		Vector3(-60.0, 6.4, -281.0), Vector3(-67.0, 6.8, -284.0),
		Vector3(-76.0, 6.0, -282.0), Vector3(-86.0, 6.5, -288.0),
		Vector3(-61.0, 6.3, -292.0), Vector3(-70.0, 6.7, -298.0),
		Vector3(-84.0, 6.0, -305.0), Vector3(-58.0, 6.5, -305.0),
		Vector3(-74.0, 6.2, -310.0)]
	for index in range(sites.size()):
		var site: Vector3 = sites[index]
		var holder := Node3D.new()
		holder.name = "NorthGroveNativeT2Oak%02d" % index
		holder.position = Vector3(site.x,
			_north_sector_surface_y(site.x, site.z, x_bands, z_bands), site.z)
		add_child(holder)
		var visual := Node3D.new()
		visual.name = "SharedT2OakVisual"
		var scale_value := site.y / 0.9941409826
		visual.scale = Vector3.ONE * scale_value
		visual.position.y = 0.4960939884 * scale_value
		holder.add_child(visual)
		visual.add_child(packed.instantiate())
		var trunk := StaticBody3D.new()
		trunk.name = "GroveTrunkOnlyCollision"
		var shape := CollisionShape3D.new()
		var cylinder := CylinderShape3D.new()
		...
		holder.set_meta("asset_path", NATIVE_FOREST_OAK_TRIAL_PATH)
		holder.set_meta("source_triangles", 14783)
		holder.set_meta("authored_display_height_m", site.y)
		holder.set_meta("trunk_proxy_only", true)
		_north_grove_trees.append(holder)
		_tree_instances += 1
		_vegetation_instances += 1
...
func _make_small_label() -> void:
	...
	var short_tour_label := (_tour_version in ["v2", "v3"] and
...
func layout_metrics() -> Dictionary:
	return {"world_extent_m": [420, 460], "ground_centre_z_m": -5,
		...
		"north_grove_trial_enabled": _north_grove_trial,
		"north_grove_additional_native_t2_oaks": _north_grove_trees.size(),
		"north_grove_bounds_m": {"x": [-90, -55], "z": [-313, -273]},
		"isolated_cottage_rebake_trial_enabled": _cottage_rebake_trial,
		"isolated_cottage_rebake_instances": 1 if _cottage_rebake_visual != null else 0,
		"isolated_cottage_rebake_target": _cottage_ab_house.position if _cottage_ab_house != null else Vector3.ZERO,
...
func _capture_north_grove_probe() -> void:
	DirAccess.make_dir_recursive_absolute(_north_grove_probe_dir)
	# Exactly the same three camera poses in baseline and candidate builds.
	# The adopted single foreground oak, old kit line, ground and light persist.
	var shots := [
		["country_road_eye", Vector3(0.0, 1.72, -222.0), Vector3(-73.0, 11.0, -290.0)],
		["forest_edge_walk_eye", Vector3(-31.0, 2.05, -255.0), Vector3(-69.0, 11.5, -293.0)],
		["forest_bank_side_eye", Vector3(-117.0, 7.4, -285.0), Vector3(-70.0, 12.0, -297.0)]
	]
	var saved: Array[String] = []
	var rows: Array[Dictionary] = []
	for shot in shots:
		...
		var image := get_viewport().get_texture().get_image()
		var name := String(shot[0])
		if image.save_png(_north_grove_probe_dir.path_join(name + ".png")) == OK:
			saved.append(name)
		rows.append({"name": name, "camera": shot[1], "target": shot[2],
			...
			"primitives_with_shadows": perf["primitives_with_shadows"]})
	var image_size := get_viewport().get_texture().get_image().get_size()
	var report := {"suite": "northwest_35_by_40m_grouped_native_oak_actual_world_trial",
		"scene": "res://scenes/floor1_expanded_world.tscn",
		"group_enabled": _north_grove_trial,
		"group_bounds_m": {"x": [-90, -55], "z": [-313, -273]},
		"candidate_additional_instances": _north_grove_trees.size(),
		"foreground_approved_native_oak_instances": 1 if _native_forest_oak_trial != null else 0,
		"original_nw_kit_trees": _north_sector_trees.size(),
		"approved_town_100k_oak_instances": 1 if _courtyard_oak != null else 0,
		"additional_source_triangles": _north_grove_trees.size() * 14783,
		"shared_asset_path": NATIVE_FOREST_OAK_TRIAL_PATH,
		"camera_mode": "actual non-headless same-world off/on",
		"capture_png_px": [image_size.x, image_size.y],
		"saved_shots": saved, "shot_rows": rows,
		"frame_interval_definition": "frame_post_draw includes VSync/presentation and CPU waits; not FPS promise",
		"world_state_mutations": 0, "model_calls": 0}
	var file := FileAccess.open(_north_grove_probe_dir.path_join("capture.json"), FileAccess.WRITE)
	if file != null:
		file.store_string(JSON.stringify(report, "  "))
		file.close()
	print(JSON.stringify({"suite": report.suite, "enabled": report.group_enabled,
		"saved": saved.size(), "png_px": report.capture_png_px}))
	get_tree().quit(0 if saved.size() == shots.size() and file != null else 1)


func _capture_cottage_rebake_probe() -> void:
	DirAccess.make_dir_recursive_absolute(_cottage_rebake_probe_dir)
	if _cottage_ab_house == null:
		push_error("Isolated cottage A/B has no west market frontage target")
		get_tree().quit(1)
		return
	var centre := _cottage_ab_house.global_position
	var front := (_cottage_ab_house.global_basis * Vector3(0, 0, 1)).normalized()
	var right := (_cottage_ab_house.global_basis * Vector3(1, 0, 0)).normalized()
	var shots := [
		["front_close_shutter_flower_box", centre + front * 8.0 + right * 4.0 + Vector3(0, 3.5, 0), centre + Vector3(0, 5.0, 0)],
		["back_roof_close_chimney_pickup", centre - front * 9.0 + right * 4.0 + Vector3(0, 8.8, 0), centre + Vector3(0, 7.8, 0)],
		["front_mid_actual_street", centre + front * 16.0 + right * 5.0 + Vector3(0, 3.0, 0), centre + Vector3(0, 5.0, 0)]
	]
	var saved: Array[String] = []
	var rows: Array[Dictionary] = []
	for shot in shots:
		...
		var image := get_viewport().get_texture().get_image()
		var name := String(shot[0])
		if image.save_png(_cottage_rebake_probe_dir.path_join(name + ".png")) == OK:
			saved.append(name)
		rows.append({"name": name, "camera": shot[1], "target": shot[2]})
	var image_size := get_viewport().get_texture().get_image().get_size()
	var report := {"suite": "one_west_market_cottage_60k2k_uv_rebake_actual_world_light_ab",
		"scene": "res://scenes/floor1_expanded_world.tscn",
		"candidate_enabled": _cottage_rebake_trial,
		"candidate_asset": COTTAGE_REBAKE_CANDIDATE_PATH if _cottage_rebake_visual != null else "",
		"candidate_imported_triangles": int(_cottage_rebake_visual.get_meta("actual_imported_triangles", 0)) if _cottage_rebake_visual != null else 0,
		"candidate_imported_surfaces": int(_cottage_rebake_visual.get_meta("actual_imported_surfaces", 0)) if _cottage_rebake_visual != null else 0,
		"candidate_raw_bounds_low": _cottage_rebake_visual.get_meta("raw_bounds_low", Vector3.ZERO) if _cottage_rebake_visual != null else Vector3.ZERO,
		"candidate_raw_bounds_high": _cottage_rebake_visual.get_meta("raw_bounds_high", Vector3.ZERO) if _cottage_rebake_visual != null else Vector3.ZERO,
		"art_display_height_m": 10.0,
		"target_house_name": _cottage_ab_house.name,
		"target_house_world_site_m": centre,
		"target_house_yaw_deg": _cottage_ab_house.rotation_degrees.y,
		"target_default_lod_asset": _cottage_ab_house.get_meta("asset_path", ""),
		"other_exterior_lod_houses_untouched": exterior_houses.size() - 1,
		"on_pass_hidden_baseline_still_loaded": _cottage_rebake_trial,
		"this_is_not_resource_memory_or_performance_proof": true,
		"actual_saved_png_px": [image_size.x, image_size.y],
		"same_camera_shots": rows, "saved_shots": saved,
		"world_state_mutations": 0, "model_calls": 0,
		"candidate_material_limit": "new 2K baked color only, matte roughness0.78; no PBR normal/ORM re-bake",
		"candidate_known_geometry_color_limit": "white atlas pickup near chimney; original highpoly roof had no such white marks"}
	var file := FileAccess.open(_cottage_rebake_probe_dir.path_join("capture.json"), FileAccess.WRITE)
	if file != null:
		file.store_string(JSON.stringify(report, "  "))
		file.close()
	print(JSON.stringify({"suite": report.suite, "enabled": report.candidate_enabled,
		"saved": saved.size(), "png_px": report.actual_saved_png_px,
		"candidate_tris": report.candidate_imported_triangles}))
	get_tree().quit(0 if saved.size() == shots.size() and file != null else 1)
