...
func _new_trade_collection_offer_due(id: String, record: Dictionary) -> bool:
	## A completed collection contract can become available after the owner walks
	## into handoff range. Treat that newly visible action as one observation
	## trigger; if the owner declines it, the ordinary cooldown still applies.
	if record.get("status", "") != "settled" or not town.has_method("action_options"):
		return false
	var offered: Variant = record.get("offered_actions")
	if not offered is Dictionary or offered.is_empty():
	...
	for alias in offered:
		var option_id: Variant = offered[alias]
		if option_id is String and not option_id.is_empty():
			previous_ids[option_id] = true
	for option in town.action_options(id):
		if option is Dictionary:
			var option_id: Variant = option.get("id")
			if option_id is String and option_id.begins_with("contract:collect:") and not previous_ids.has(option_id):
				return true
	return false
