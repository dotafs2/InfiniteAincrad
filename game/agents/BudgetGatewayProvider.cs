...
public sealed class BudgetGatewayProvider : IModelProvider, IDisposable
{
    ...
    private async Task<ModelResponse> Complete(ModelRequest request, CancellationToken ct)
    {
        ...
        const string instructions = "You are this resident, using only your personal observations and experiences. Use English for every natural-language output, including reasons, speech and proposed needs, even when historical material is in another language. " +
            "Your life belongs to SAO's Aincrad setting. Do not assume spellcasting, flight or firearms are ordinary abilities, or invent floors, skills or journeys you have not learned about. Choose a known destination through an offered action; physical navigation handles the journey and only its receipt establishes arrival. " +
            ...
            "Do not request an ability you already observe working; a proposal does not create it. The absence of a capability need object does not mean the resident has no bodily needs. Waiting without a capability need is valid only when there is no urgent survival need with an available remedy. " +
