...
flowchart TD
    ...
    H55["H55 原GM10基础动作可见性（已实现）<br/>权威动作与工作事实离线已验；待实机GM消费"]
    H55 --> M20H
    M20E --> M20
    M20 -.持续稳定性.-> M20F
    H20 --> M20G
...
| H55 | 原GM10从已提议方案实际认领issue-266e1c4096de，单文件导出基础动作、计时、位置与目标距离，正常任务不称卡住；独立读取与精确数值检查通过。[事实](docs/validation/gm_home_return_2026-09-14.md)。 | **部分：已实现并准备同档安装，待真实运行记录被GM消费**。原有64条上限与诊断优先级保留；编码两次未知费用保留，不把离线fixture当实机采用。 |
