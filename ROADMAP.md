...
flowchart TD
    ...
    M20H["M20H GM持续观察世界<br/>待接通 · 场景/碰撞/行动异常证据、问题去重；与NPC感知分离"]
    H25["H25 源码确认：居民调用仍受短验证限制<br/>每适配器12次、网关运行配置最多32次；持续运行待实现"]
    H26["H26 源码缺口：受阻任务屏蔽居民新决策<br/>等待到达才能完成，缺生活失败反馈与自愿改选"]
    ...
    class H5,H17,N3,H24,H25,H26,H27,H28,M20A,M20B,M20C,M20E,M20F,M20G partial;
...
| M20H | 现有物理射线、截图和专项探针，尚无持续向GM发送运行问题的通道；[源码复核](docs/ARCHITECTURE.md) | 给GM对象ID、场景版本、位置/目标、碰撞与无进展记录、行动结果等可复核证据；按问题去重认领。区分可穿过草叶与墙体穿透、合理拒绝与故障。画面类异常须另验图像链路，不默认DeepSeek模型已能看图。 |
...
| H26 | 调度跳过所有`pending_job`；材料等工作只在到达后推进，封闭时尚无自愿停止；源码与H24封闭样例一致 | 产生本人可理解的受阻反馈，让Kimi自行等待/停止/改选；GM看技术诊断。不是把开发信息发给NPC，也不强制替其选择。 |
