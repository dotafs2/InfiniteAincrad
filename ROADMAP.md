...
- **H125 engineering pivot:** the saved Carpenter body stopped 0.86 m from the finite source because the physical capsule route can reject the exact source center even though the world's work gate accepts any position within 0.45 m. Material steering now tries a collision-cleared approach point 0.40 m from the source before declaring the trip unreachable. This keeps the authoritative destination, collision sweep, 0.45 m work gate and finite accounting unchanged. A valid Godot binary was not available in this checkout for a scene replay; the change is therefore code-reviewed and `git diff --check`-clean, pending the next owned scene run.
