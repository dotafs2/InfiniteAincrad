...
flowchart TD
    ...
    H53 --> M20E
    H53 -.老师返家动作阻塞.-> H54
    H54["H54 原GM09基础动作返家寻路（离线已验）<br/>四个原进食/休息任务完整场景完成；待Kimi同档采用"]
    H54 --> M20E
    H54 -.基础pending不在GM投影.-> H55
    H55["H55 原GM10基础动作可见性（已提议）<br/>让GM区分模型回答落定、身体行进与实际工作"]
    H55 --> M20H
    M20E --> M20
    M20 -.持续稳定性.-> M20F
    H20 --> M20G
...
| H54 | GPT-6从原世界约1721秒基础动作未完成发现问题；原GM09认领并写两文件道路返家修复。独立原速完整城镇45项通过，四个原进食/休息动作一次完成并冷恢复。[事实与宿主纠正](docs/validation/gm_home_return_2026-09-14.md)。 | **部分：离线原档副本通过，待真实Kimi同档采用**；模型settled不能替代身体完成，不宣称全部拥堵已解决。 |
| H55 | 原GM10真实提议一文件基础pending动作投影：权威动作、计时、位置及目标距离，明确未证明卡住，不泄露居民思想。[同一报告](docs/validation/gm_home_return_2026-09-14.md)。 | **待编码认领与实机消费**：是宿主发现的观察缺口，现有GM投影仍不能覆盖基础进食/休息。 |
