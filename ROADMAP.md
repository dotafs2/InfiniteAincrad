...
| H42 | **离线有界绿（仅限已测控制范围）**：`--stop-on-decision-limit` 转发 `--town-stop-on-decision-limit`。上限前合法空闲照常推进世界时间；到达决策上限后不再多发请求；在途模型结果及其账本/事务落定后立即暂停、capture并退出，不等长途或受阻的物理任务；同一未决命令的ID、目标、进度与历史原样保留，独立冷进程可续接并只完成一次。默认与`--town-stop-on-idle`、900秒/32次上限、1800秒冷却、调度、原账本/费用/未知与不重试语义均未改。作者与独立评审各19+99=118项断言0失败，5棵自有进程树全部退出。 | **限制（保留）**：启动器转发只有mock进程单测，Godot侧由fixture直接设定控制器字段；没有启动器→编译适配器→假网关的端到端cap episode，也没有任何真实Kimi cap-only运行。此前的`unapproved/undeployed`指root接受前的工程产物状态，不是产品缺陷。 |
| H43 | 过期选项后的真实重规划（待验）：第三轮真实续接中旅店主`turn:shared:innkeeper:0:18`（`ask:shared:smith`）因目标已变化返回`option_unavailable`，按`stale_option_v1`保存且未重放；当时世界时间978.5667，`not_before=2733.6167`。 | **待验、不标绿**：需同一身份在同一1800世界秒冷却阈值之后自然获得新的请求/选项与既有拒绝反馈，并产生新的权威结果；不得强制选择或重放turn18。这是验收缺口，不是已证缺陷；没有新的真实运行发生。 |
