...
flowchart TD
    ...
    H49 --> M20C
    M20H -.铁匠真实行程长期停滞.-> H50
    H50["H50 原GM01靠近失败边界（离线已验）<br/>真实碰撞、慢速进展与冷恢复通过；待同档采用"]
    H50 --> M20E
    M20B -.木匠错误类别丢失.-> H51
    H51["H51 原GM03模型错误分类（离线已验）<br/>明确类别净化落盘；旧失败原因仍未知"]
    H51 --> M20H
    M20E --> M20
    M20 -.持续稳定性.-> M20F
    H20 --> M20G
...
| M20H | H29/H30/H41原有投影基础保留；本轮原GM01/02/04/07已消费真实seq169证据，形成行程停滞、真实渔夫技能需求和牧羊人本地错误三项认领；H48/H49实际代码进入同档并被采用。 | 其余六个原GM继续接收seq180新证据；木匠新错误原因仍未确定。持续全员观察、真实图像类异常和自动完整交付仍待验，不能把名单或一次预检称为完成。 |
...
| H50 | 原GM01依据真实铁匠停滞认领并完成有界靠近失败；原真实场景48项、交易63项、边界48项、连续慢行223项、微漂移102项通过，同档副本冷恢复通过。旧永久挂起断言按新行为更新后，社交物理36项通过。[验收和退回记录](docs/validation/gm_approach_error_class_2026-09-14.md)。 | **离线通过，待同档采用**：无进展时只记录一次未到达失败，不宣称穿模根因已解决，也不保证居民一定换一个目标。 |
| H51 | 原GM03依据真实木匠错误，自主认领并实现可核实异常的净化分类；16项分类与落盘、17项既有控制器恢复及完整解析通过。[验收事实](docs/validation/gm_approach_error_class_2026-09-14.md)。 | **离线通过**：旧木匠失败原因仍未知，分类不会重写历史或自行重试。须由新证据决定下一步修复；正常冷却和原费用门槛保留。 |
