...
flowchart TD
    ...
    H49 --> M20C
    M20H -.铁匠真实行程长期停滞.-> H50
    H50["H50 原GM01靠近失败边界（补丁待同档采用）<br/>实际续跑暴露关闭记录超限；原GM修复保存事务"]
    H50 --> M20E
    M20B -.木匠错误类别丢失.-> H51
    H51["H51 原GM03模型错误分类（首次实际采用）<br/>木匠新输入超限准确落盘；原GM06消费并修复"]
    H51 --> M20H
    H51 -.真实输入超限.-> H52
    H52["H52 原GM06个人历史输入压缩（离线已验）<br/>保留身份、行动和承诺；待原木匠新Kimi请求"]
    H52 --> M20B
    M20E --> M20
    M20 -.持续稳定性.-> M20F
    H20 --> M20G
    ...
    class H47 partial;
    class H48,H49,H51 done;
    class H50,H52 partial;
    class H38 partial;
    class H39 done;
    class H43 partial;
    class H33 done;
...
| H50 | 原GM01有界靠近通过原真实场景48项、社交36项、慢行223项及微漂移102项；实际life03暴露最后一条行程关闭后25条已关闭记录超过24条保存上限。原GM01修复同一事务内裁剪；真实TownPlaces副本29项及原场景48项通过。[失败和修复](docs/validation/gm_approach_error_class_2026-09-14.md)。 | **修复离线通过，待同档采用**：life03只推进89.9世界秒即保存失败，不能称15分钟成功；不宣称穿模根因或所有拥堵已解决。 |
...
| H52 | 原GM06只压缩发送副本的个人历史说明及较旧条目，身份、规则、行动、当前财物和承诺完整保留；27项独立边界测试、22项原输入测试、16项分类与29项控制器检查通过。[边界及测试纠正](docs/validation/gm_approach_error_class_2026-09-14.md)。 | **离线通过，真实采用待验**：按16000 UTF-16单位核算；不可再缩的必要信息超限时本地拒绝且不调用模型。原存档历史未删改，须由原木匠新请求验证。 |
