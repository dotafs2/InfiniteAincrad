...
| M20E | N4/H12已完成一次专项能力开发、同档安装与居民真实采用；[首条真实闭环](docs/validation/real_ten_2026-09-12/report.md)已完成“真实观察→GM认领→实现/测试/修错→独立复核→同档发布→居民自然完成原命令→冷恢复”，世界sha `7f2c91a4…`→`ea96edea…` | **首次一项闭环已在有界范围标绿（H37）**：同一世界、同一命令、只完成一次、无传送或伪造回执。后续追加：至少两个独立GM贡献的组合、依赖/版本/空间/资源冲突与真实采用。失败只退候选，不整档回滚抹除已经发生的生活事实。 |
...
| M20H | [H29世界/需求证据入口](docs/validation/town_gm_evidence_2026-09-12.md)离线验收：持续更新、问题关闭移除、提议去重、历史/意图并存与个人上下文隔离；H30的10个真实GM已消费测试投影 | 实际居民需求和运行缺陷的认领、修复仍待验；H33已采集实际接触对象、物理绕行与同档结果；真实GM诊断/修复这些问题及图像类异常仍待验证。 |
...
| H36 | [首条真实闭环](docs/validation/real_ten_2026-09-12/report.md)：木匠胶囊阻挡已由实际物理验证并修复，life/trade 待办计数已补齐，同档完成与冷恢复见 H37。原始证据保留：同一 approach 命令相隔 93.5167 秒位置不变，距目标 1.1503 米；冻结投影 seq 1（该次世界 seq 17）遗漏此任务，不代表最终全世界快照。首轮 6 份有效、4 份格式失败；原会话重试后十个 GM 观察均有效，GM09 实际实现及交付另见 H37。 | **部分完成、继续开放**：尚缺通用 approach 停滞/阻挡诊断进入后台 GM 导出的通道。下一验收是在真实运行中输出有来源的社交受阻证据，验证去重、更新与居民隐私边界；物理绕行、待办计数和同档续接已完成。 |
