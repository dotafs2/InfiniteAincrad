...
flowchart TD
    ...
    H31["H31 源码缺口：GM新问题提议入口受限<br/>无既有issue不调度；新ID输出被拒绝；先接有来源的最小调查"]
    N3 -.整合发现.-> H18
    N5 -.跨机器整合.-> H19
    H19 -.实机发现.-> H20
    H5 -.有来源的直接交流.-> H21
    ...
    class H5,H17,N3,H25,H27,H28,M20A,M20B,M20C,M20D,M20E,M20F,M20G,M20H partial;
...
| M20A | 居民稳定ID、加入去重与同档恢复有[三人接入证据](docs/validation/town_online_2026-09-11.md)；H30已登记10GM观察身份；十人新世界候选的保存/恢复仍失败，未验收 | 本轮新建独立持续保存的十人试运行世界，创建即固定身份/资源来源，不冒充原镇S1恢复。接通前核对十身体、初始化陈述与实际能力及冷恢复；重连不重复创建人物/发钱。 |
...
| M20D | [H30真实观察](docs/validation/town_gm_observers_2026-09-12.md)：10个不同DeepSeek会话消费测试世界导出，65个完整响应，重复同一证据0调用；29项执行器假传输测试通过 | 部分完成：实际GM没有认领占位需求，尚无真实代码候选/发布/居民采用。接入真实问题或注明来源的主动改进，验证所属GM实施、测试修错及同档交付；真实变化证据的会话续接与持续调度待验。 |
| M20E | N4/H12已完成一次专项能力开发、同档安装与居民真实采用；当前GM候选/统一发布/采用链未实跑 | 首次共同运行：至少一项GM修复经审核进入同一世界，观察居民有来源感知与实际续接，采用/拒绝分别记证。后续追加：至少两个独立GM贡献的组合、依赖/版本/空间/资源冲突与真实采用。失败只退候选，不整档回滚抹除已经发生的生活事实。 |
...
| M20H | [H29世界/需求证据入口](docs/validation/town_gm_evidence_2026-09-12.md)离线验收：持续更新、问题关闭移除、提议去重、历史/意图并存与个人上下文隔离；H30的10个真实GM已消费测试投影 | 实际居民需求和运行缺陷的认领、修复仍待验；现有无进展证据不能证明碰撞缺陷，接触点、对象识别和图像类异常仍待验证。 |
...
| H31 | [三方源码互审](docs/validation/route_review_2026-09-12.md)：无既有问题时`plan_observation`不派发，GM输出只接受已有issue ID | 先用一项真实need/运行问题或注明来源的有界主动调查接通修复。新发现保留证据和发现者，需监督登记的环节如实标注；只补最小提议入口，不前置通用巡检平台。 |
