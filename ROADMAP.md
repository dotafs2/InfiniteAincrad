...
flowchart TD
    ...
    H33["H33 实测采集拥堵<br/>部分：8个原任务同档完成；最终障碍负例原生崩溃"]
...
| M20A | [本轮独立十人世界验收](docs/validation/gpt6_sprint_2026-09-12.md)：创建拒绝覆盖、10个实际身体、原任务跨进程恢复；维护中的试运行种子保持未运行，隔离测试副本另存 | 身份/初始化与十身体已离线验收；首次真实模型运行必须延续所选主档和原账本。原镇S1仍未恢复，不能把独立新世界冒充原镇。 |
...
| M20D | H30十个真实DeepSeek观察会话已验；H31新增有来源调查→GM提议/认领→失败修错→候选，离线传输验证；H32/同档材料链已有实际规则后果 | 真实模型认领与开发、审核发布、居民采用仍待验；脚本上下文和配置候选不是自主10+10运行证明。 |
| M20E | N4/H12已完成一次专项能力开发、同档安装与居民真实采用；本轮脚本传输下的候选/审核/同档采用链已验，真实模型链尚未实跑 | 首次共同运行：至少一项GM修复经审核进入同一世界，观察居民有来源感知与实际续接，采用/拒绝分别记证。后续追加：至少两个独立GM贡献的组合、依赖/版本/空间/资源冲突与真实采用。失败只退候选，不整档回滚抹除已经发生的生活事实。 |
| M20F | 三人同档冷恢复、控制器epoch/去重与部分故障注入已有；10+10实际共同运行和服务指标未测 | 首次：有结束条件的真实10+10、一次带未完任务的恢复及受控更新续接，身份/钱物/历史可核对。后续追加：长时运行、完整故障隔离/冲突注入与容量覆盖；已有未知费用继续保留，不重复核销，记录等待/主机负载/帧时。 |
...
| H33 | [十人实测](docs/validation/gpt6_sprint_2026-09-12.md)：7个相同采集任务20秒工时不变，身体仅移动约1—15毫米并持续接触，食物库存始终大于0 | 部分验收：8个原任务同档全部结束，2人取得剩余食物、6人真实库存耗尽，未重置任务或资源。最终障碍负例两次发生H34原生崩溃，不能标为全面通过。 |
| H34 | [原生崩溃与恢复记录](docs/validation/gpt6_sprint_2026-09-12.md)：Godot访问冲突0xc0000005；原任务39.27秒检查点保留，受控恢复后完成 | 原因未明，成功恢复不代表稳定性通过；同一异常偏移的原生日志和匹配Windows事件保留。 |
