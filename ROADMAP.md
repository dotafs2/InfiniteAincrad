...
近期不再把多电脑同时开发、外部NPC在线加入、GM在线安装、通用热更新或多人规模作为前置。保留已有模块划分、去重和控制器隔离代码；多人开源协作仍是远期参与方式，有实际瓶颈再决定并行程度，不把所有并行永久判定为无价值。公开试玩仍以可玩和可恢复为准。
...
flowchart TD
    ...
    H20["H20 实机发现：日照过曝与居民标签重叠；对比实机修复，尚未完成"]
    ...
    class H5,H17,N3 partial;
    class H18,H19,H21 done;
    class H20 issue;
    class S1 issue;
...
| H5 | 玩家线索有效；直接声明机制已由 H21 通过（95项专测），居民独立获知镇内他人技能的证据仍不足 | 需要有来源的交流或观察渠道，不能注入全镇知识；转介与自主发现仍待验证。 |
...
| H20 | 实机发现：日照过曝与居民标签重叠；对比实机修复，尚未完成 | [跨机器报告](docs/validation/cross_machine_2026-09-12.md) |
| H21 | 自愿技能介绍与有来源的个人记忆已验证；95项离线专测＋26套件；[证据](docs/validation/town_skill_notice_2026-09-12.md) | 真实自主发现与第三方转介待验；H5 仍为部分完成。 |
...
下文的长期内容里程碑仍可参考；其中在线接入、热更新和常驻服务器路径是旧方案的可选研究，不再驱动当前执行顺序。
