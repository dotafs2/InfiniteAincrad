...
**这是用户最新确定的近期里程碑 M20，覆盖下文“多人规模与 GM 共建只属远期”的旧排序。** 十名 Kimi 居民在同一个正式、持续保存的世界里生活；十个独立 AI GM 在世界之外持续维护、开发这个游戏，包括修复缺陷、补全居民缺少的能力，以及主动提出有依据的世界改进。GM 可以由不同维护者提供，保留各自身份、责任、任务和贡献历史；他们不是额外十名普通 NPC，也不靠在街上摆二十个模型完成目标。

居民不知道后台的代码、派工、测试日志、模型用量或 GM 私人讨论。居民可表达“我缺材料/无法完成某件事”等生活需求，后台把真实需求转成开发任务；新能力进入世界后，居民通过本人观察、有来源的消息和实际使用得知变化。**开发过程不进入 NPC 上下文，不等于删除旧记忆、注入成功经历或让居民永远看不见世界变化。** GM 也可主动改进，但要标明 `author_proposal` / `gm_world_plan`，不能伪造居民需求。
...
目前证据仍是三人测试世界、受限控制器接入，以及一次由开发代理促成的新结算能力；**M20 尚未完成**。下面新增的 M20A—M20G 是它的前置工作，状态依据现有证据标注；验收条件是待执行的目标，不是已经通过的结果。
...
flowchart TD
    ...
    H24["H24 本地已验收：材料短距离绕障与原任务续接<br/>3782帧无穿透；封闭时保留待办；发布收尾待完成"]
    M20["M20 近期正式共同世界：10名Kimi居民 + 10个后台AI GM<br/>尚未验收 · 居民持续生活，GM持续开发，NPC不接收开发内部信息"]
    ...
    M20D["M20D 十个独立AI GM持续开发<br/>待实现 · 真实需求/主动提案、认领、隔离实现测试、持久任务履历"]
    ...
    class H5,H17,N3,H24,M20A,M20B,M20C,M20E,M20F,M20G partial;
    class H18,H19,H20,H21,H22,H23 done;
    class S1 issue;
    class M20,M20D,P,N5,N6,N7,N8,N9,XR,N10,R future;
```
...
| H24 | 本地未提交的专项结果：从同一阻塞测试档的逐字节副本续接原取料任务；3782帧检测无穿透，库存2→1/铁2→3；封闭障碍保留待办和资源；28套件与3校验器通过 | 橙色：正式报告、证据归档和源码发布尚未完成。本次规划不发布该实现。只涉及材料短距离绕障，不是全城导航；仍缺自愿停止受阻任务。 |
| M20 | **目标待验：10名正式Kimi居民生活 + 10个独立后台AI GM维护开发同一世界** | 以本页“首次10+10验收”整体判断，不能由单项绿色或“启动了20个进程”推导通过。 |
...
| M20D | [架构文档](docs/ARCHITECTURE.md)仅有GM贡献设计；现有专项安装与一次开发代理回应不构成10个常驻GM | 10个独立后台AI GM持有各自职责、任务/恢复记录和受限工具；来自真实居民困难或明确主动提案，认领有边界任务，自行实现→测试→修错，提交候选成果或明确阻塞。任务中断可继续，不重复认领同一修改范围。开发日志和派工不进入NPC观察。 |
...
以下是本轮规划的验收基线，尚未执行；实际运行前冻结时长、模拟时间倍率、预算和机器配置。建议先完成**至少3个完整游戏日的生活循环，并累计24小时真实共同运行**；压缩模拟时间不能冒称24小时真实运行。数值可经用户讨论调整，不是已经达成的服务承诺。
...
- **GM确实在开发维护：**十个GM均能接续自己的真实任务并留下可核验产出（实现、验证、审查或维护）；不得用十个名义在线账号充数。至少两个不同GM的修改走完需求/提案→隔离开发→测试→冲突检查→发布→居民实际采用。GM有任务时推进，无任务时等待，不为凑活跃量制造无用功能。
...
最强反对意见：把人数先凑满会放大供给不足、无效思考和互相覆盖，十个GM也可能只是在重复做同一个功能。反例是两个GM各自测试通过，却同时占用同一地块/资源，或覆盖了正在履行的合同规则。因此最小有用验证是**当前三人 + 两个独立后台GM**：一个实际居民困难、一个主动提案；两项成果分别验证一次真实采用，并让一个冲突候选明确失败且正式档不变。之后按3居民+2GM→5+3→10+10扩大，同一正式主档继续；正式档建立之前的演练明确使用测试档。
...
近期顺序：完成H24发布收尾；完善DeepSeek完整执行循环与实测缓存记录；明确M20A正式主档来源；先验证三人+两个独立后台GM的需求兑现/冲突隔离，再补十人供给与容量，逐步完成M20。这里的规划不启动任何新的付费居民或常驻GM循环，正式运行使用另行确定的实际范围、配置和预算；已结束的夜间任务不自动恢复。
