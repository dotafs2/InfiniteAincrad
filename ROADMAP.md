...
flowchart TD
    ...
    H48 --> M20E
    M20B -.牧羊人旧本地次数限制.-> H49
    H49["H49 原GM07本地错误冷恢复（部分）<br/>按请求限定一次恢复、原历史保留；真实新请求待验"]
    H49 --> M20C
    M20E --> M20
    M20 -.持续稳定性.-> M20F
    H20 --> M20G
...
| H48 | 原GM02从原Kimi渔夫真实提议 `ask_carpenter_help` 自主认领并实现具体技能求助。独立验收25项求助、63项交易、55项答复/取消/恢复检查通过；首次场景启动暴露继承签名不兼容，原GM02修复后TownRuntime完整继承链解析及25＋63项通过。首段Kimi0请求、原档不变；源码仅town_trade.gd。[证据与限制](docs/validation/gm_skill_help_2026-09-14.md)。 | **部分**：发布到同一世界后仍需Kimi自主选择并获得真实结果。该功能只允许提出具体技能求助，不会自动完成木工或强制对方接单。GM原运行费用未知尾部保留，最终runner的usage_incomplete不伪装成成功。 |
| H49 | 原GM07读取牧羊人实际 `brain_session_request_limit` 记录并自主认领、实现。冷恢复48项、不同后续错误19项、原调度29项宿主检查通过。[事实与限制](docs/validation/gm_local_limit_2026-09-14.md)。 | **部分**：仅明确的旧本地次数限制允许冷启动后一个新请求；即时重复失败、其他模型错误/在途/断线/冷却仍隔离。保留原错误与历史，不重放。Kimi牧羊人真实恢复与同档结果待验。 |
