...
The active lineage has since advanced to seq263 with 243 world usage records and CNY 2.0092732 of additional Kimi cost after the earlier baseline. H126's 12 calls settled with no unknowns; its 600-second capture resolved and drained internally, but the outer runner timed out while closing. Carpenter still remains physically blocked 0.715 m from the exhausted source with 5.6833/60 seconds on the same command. H127 adds a non-paid measured street-graph fallback for the live route; this is an implementation fix, not yet a live depletion or repair result.
