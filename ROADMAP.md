...
flowchart TD
    ...
    class H37,H40,H41,H42,H44 done;
    class H38 partial;
    class H39 done;
...
| H43 | 过期选项后的真实重规划（待验）：第三轮真实续接中旅店主`turn:shared:innkeeper:0:18`（`ask:shared:smith`）因目标已变化返回`option_unavailable`，按`stale_option_v1`保存且未重放；task33当时世界时间978.5667；task41续接后的最新时间1278.2333仍低于`not_before=2733.6167`。 | **待验、不标绿**：需同一身份在同一1800世界秒冷却阈值之后自然获得新的请求/选项与既有拒绝反馈，并产生新的权威结果；不得强制选择或重放turn18。这是验收缺口，不是已证缺陷；没有新的真实运行发生。 |
| H44 | task41在同一维护世界推进seq131→139，11次已落定Kimi调用、+0.3949409 CNY、不确定行6→6；Smith铁料0仍看到接受修理选项，权威提交正确返回`insufficient_funds`并守恒。原gm-02编码回合900秒超时、费用未知、无JSON合约且候选有范围外引擎元数据；保留原候选后仅提取3个许可文件，GPT-5.6修正1行测试清理错误，独立通过新验收57、交易63、合同20、H37兼容30项。 | **有界绿仅限离线修复**：缺铁/钱/技能时不列可执行接受，并给本人诚实的不可用原因；不送铁、不降价、不强制成交、不移除未修需求或权威拒绝。产品源码是gm-02所写，测试清理由GPT-5.6完成；原超时回合不称完整GM交付，实际同档采用待H45。Fisher同轮一次`brain_response_invalid`仅为未定因事实。 |
| H45 | task50已离线接受精确双控制器恢复器：新副本中按序归档Fisher `turn:shared:fisher:0:14`与Smith `turn:shared:smith:0:15`旧失败，各升epoch一次，并要求其余居民控制器（含旅店主turn18冷却）逐项不变。 | **待验、不标绿**：H44发布后才可生成并独立审核恢复副本，再由授权发布者采用；随后须看到Fisher/Smith各自首个新epoch真实请求且旧turn不重放、十身份/产权/历史/任务保全。Fisher若再次解析失败须停止复核，不自动重试。 |
