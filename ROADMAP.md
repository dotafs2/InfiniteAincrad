...
flowchart TD
    ...
    H26["H26 本地候选：受阻反馈、自愿取消与重复受阻<br/>五场景223项已通过；长ID/取消回执/十任务并存待补验收"]
...
| H26 | 本地候选：五场景223项与状态专项206项通过；包含真实物理停滞、取消后新出发、同作业重复受阻及同档冷恢复。[报告](docs/validation/town_material_blocked_2026-09-12.md) | 橙色，未接受发布：修复合法长ID的派生键上限、成功取消与原取材失败的双回执，补32条历史＋10个进行中任务同时保存恢复。仅测试控制器，真实Kimi和GM消费尚未接通。 |
