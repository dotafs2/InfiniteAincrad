...
flowchart TD
    ...
    class H5,H17,N3,H25,H27,H28,M20A,M20B,M20C,M20E,M20F,M20G,M20H partial;
    class H18,H19,H20,H21,H22,H23,H24,H26,H29 done;
    class S1 issue;
...
| M20D | [源码复核](docs/ARCHITECTURE.md)：现有专项安装与一次开发代理回应不构成10个常驻GM；H29已把被接受的`need`送入独立证据文件并保留来源；原生DeepSeek开发工具已实跑，十个GM的实际消费/认领/交付仍未接通 | 10个DeepSeek GM各有会话、职责、任务/恢复记录与工具；从运行证据或真实居民需求认领任务，自行实现→测试→修错。接通带来源的缺失能力提议；私人理由不自动当公开发言。无任务可等待，不重复修改同一范围。 |
...
| M20H | [H29世界/需求证据入口](docs/validation/town_gm_evidence_2026-09-12.md)离线验收：启动、提交的世界批次与居民回合自动更新，问题关闭移除、提议去重、原始历史/最新意图并存，个人上下文隔离；GM模型尚未消费 | 接通真实GM观察、去重认领；现有无进展证据不能证明碰撞缺陷，接触点、对象识别和图像类异常仍待验证。 |
...
| H29 | [桥接验收](docs/validation/town_gm_evidence_2026-09-12.md)：89项实际场景、278项状态、29项回合与27项反馈通过；NPC模型调用0 | 绿色仅限离线世界/需求文件通道：持续更新、来源与个人信息边界、失败可见、旧档和原始需求保留。不是GM已消费或10+10完成；下一步M20D。 |
