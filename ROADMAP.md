...
flowchart TD
    ...
    H42["H42 长时观察的决策上限退出控制（待验）<br/>空闲时继续；额度耗尽且模型结果落账后暂停保存"]
...
| H42 | 长时观察的决策上限退出控制（待验）：源码确认单次上限为900秒/32次决策；不启用`--town-stop-on-idle`时，决策额度耗尽后控制器停发而物理世界继续推进；启用时，暂时无人到期也会提前退出，无法自然跨过长冷却。第三轮旅店主的真实过期选择仅是触发验收需求的独立事实，不能据此判定缺陷。 | **待验、不标绿**：暂时空闲时继续推进；额度耗尽后只等正在进行的模型结果及其账本/事务落定，随即暂停并走现有正常capture/quit。长途或受阻的物理任务不必完成，其ID、目标和历史须原样保存供冷续接。离线验收须证明不多发请求、不遗失待办、冷恢复一致，且不改1800秒冷却、调度、账本、价格或重试语义；之后才可另行授权同世界实测重规划。 |
