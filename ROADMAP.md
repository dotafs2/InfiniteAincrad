...
更新：2026-09-14，工程检查点7c2b633及停止修复7123433已安装并实际续跑，当前seq102、十原居民；本世界累计60次已结算Kimi，上轮38次已计量GM，本轮十GM观察进行中，另1次历史GM编码费用未知保留并已承接。新增20次Kimi费用0.5569916元，身份与旧历史连续。H77运行副本6项停止/重建检查通过；原GM已实际读取档案。GM04认领网框；GM06提出回复选项显示对象姓名，426字符摘要遭旧格式拒收，长度规则已修，准备原文重处理及小改进。MVP仍未完成，没有新定时任务。
...
| 当前 M20 MVP：可重复、可恢复的自主闭环 | **部分完成，当前工作重点** | 居民生活与全局留档→GM按职责观察和记忆→GM开发/自测→主AI意见评审（重大问题才block）→交付并继续生活→原GM下一轮核对效果；居民保持自主选择 |
...
当前MVP仍需：十名NPC真实Kimi决策、十个有独立职责/记忆的GM实际工作记录；至少一项有价值的GM交付被居民自愿实际使用，原GM在后续轮次获得真实效果并继续，暂停/恢复后本测试档仍连续。新全局档案和权限不得混入普通NPC的全知输入。未采用可以是正常拒绝/等待，由GM观察后判断，不作为自动停生活理由；但主AI向用户宣布MVP完成时仍须有真实采用证据。旧世界证据不冒充本轮，24小时运行、十GM全部编码和零错误均不是前置。

## 当前执行顺序

1. **使用现有测试档。** 十居民、十GM和seq102已经存在；继续本档，不因更换工作流重新造世界或重放已付费调用。
2. **落实用户新流程：H74 / H75 / H76。** 完整居民档案及职责权限、每GM独立记忆、GM自测与主AI意见评审；解除旧固定宿主测试的交付决定权。普通阻塞立即向用户说明。
3. **统一NPC寻路：H77。** 采用适合自由移动3D城镇的导航网格与NavigationAgent3D提供路径/局部避让；复用实际场景碰撞和现有角色移动规则，不继续逐个地点硬写绕行，不用传送或只看距离宣称抵达。
4. **接回实际居民需求：H71 / M20E。** 原GM根据新证据完成一项居民有用的改进，可继续网框或选择更小合理范围；主AI给意见、仅block具体重大问题，交付后居民自选，原GM下一轮检查效果。H73历史未知已按用户继续指令recover/acknowledge，未知数值保留，原会话未重建。先修当前停止导航报错，不把完整经济或永久旧档恢复提前为MVP要求。
...
flowchart TD
    ...
    H73 --> M20F
    M20H --> H74
    H74["H74 全局原文档案与GM职责读取离线通过<br/>原档15条实际对话；新回包跨轮/重新载入通过"]
    H74 --> M20D
    M20D --> H75
    H75["H75 每GM完整记忆与分页CLI已接入<br/>来源改摘要引用；十GM默认24KB预检通过"]
    H75 --> M20D
    H69 --> H76
    H76["H76 GM自测→主AI意见评审→交付→下轮GM复查<br/>离线实现通过；重大问题才block；待真实交付"]
    H76 --> M20E
    H72 --> H77
    H77["H77 统一NPC导航与局部避让已安装<br/>行走14项；clear_route实测报错已修，副本停止/重建6项通过"]
    H77 --> M20C
    M20D -.开发消耗控制.-> H78
...
| M20D | H37与H48—H55已有原GM认领、编码及多项真实采用；本轮十GM也已实际观察，原会话与历史保留。 | **新流程待接回真实运行**：各GM按稳定职责和自己的持久记忆工作，模型可替换；GM自行测试，主AI给评审意见，仅明确重大问题block。全局居民档案按职责读取，原GM下一轮检验效果；不再固定十个DeepSeek或使用自动质量门禁。 |
| M20E | 历史H48/H49等多GM贡献已有同档采用；本轮观察工作已装入但未采用，网框候选未完成。 | **真实新采用仍待证据**。H76改为主AI意见评审后的有界版本交付，GM在下一轮看实际效果；单文件模板/固定宿主测试不再是许可条件。文件防覆盖、原档续接和作者记录用于协作恢复，不能冒称任意发布都可靠。 |
...
| H73 | **新增运行阻塞**：code-20260914T055705Z-ee75fc于14:12:05被主代理配置的900秒宿主超时终止；0个completed turn、usage/cost未知，状态timeout_unknown，原GM04会话和部分候选保留。不是已结算局部失败。 | 保留原run/session/费用未知，不补零、不重放。进程job已全部退出，tracked/HEAD冻结与seq79世界基线守住，部署247摘要未改。可信结算核对或明确承接此未知的新范围后才恢复付费；先缩小任务并提供物理前提，时限内交回已计量结果或明确未完成，不削弱guard。 |
| H74 | **离线通过，待下一轮真实回包**：旧档普通GM能读15条送达对话，GM02能读55条对话与决定；档案/CLI10项通过。 | TownRuntime存档副本probe正常原文及失败回包均可跨下一轮重新载入，legacy不重复、读档不改字节；原文解析及原回合29项通过。历史缺失仍明确标记，不补造原文。 |
| H75 | **记忆/分页及默认提示预算已收尾**：旧来源保留磁盘，提示使用摘要与准确引用；十GM默认24KB dry-run通过，实际23036—23836字节。 | 换session、超过12条、同run合并和原GM04续跑说明保留已检查；按UTF8字节分配预算。真实下一轮使用仍待运行。 |
| H76 | **离线实现通过，待真实交付**：record-review CLI已核对，175项旧回归及最终22项定向通过；主AI意见持久返还原GM，partial resume失败恢复已修正。 | GM自测→主AI给意见，具体重大问题才block→交付继续生活→原GM下一轮复查；默认不执行固定宿主测试门禁，保留版本、作者和文件防覆盖。 |
| H77 | **已安装，14项实碰撞检查通过**：市场/下方双向、西侧居民回市场、水槽附近均到达0.45m内，地图外目标停止；均是副本上的路线检查。 | NavigationMesh与NavigationAgent3D基于实际静态碰撞；RVO保留安全速度，部分路径不足以冒称可达，导航未就绪停止。实际NPC自主运行待下一轮。参考[Godot官方导航文档](https://docs.godotengine.org/en/stable/tutorials/navigation/navigation_introduction_3d.html)。 |
