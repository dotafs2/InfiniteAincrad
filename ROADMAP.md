...
模型/接口实验更新：2026-09-15，H85完成四模型×两入口×三情境×两重复的48分支对照；46结束观察、2服务异常，48冷恢复保持。当前建议先改善执行反馈和中断改计划，再扩大工具；正式世界仍seq129暂停，见下方[H85实测结论](#model-interface-results)。
...
| [tools/model_interface_experiment.py](tools/model_interface_experiment.py)、`game/experiments/` | H85隔离模型/接口对照：复制明确存档、调用指定API或Codex订阅、实际执行与冷恢复；汇总和离线重放工具也在`tools/`。不接管正式世界或GM发布。 |
...
H85已完成H84建议的接口对照：当前优先改善真实执行反馈和受阻任务的中断/改计划，再决定是否增加技能库或扩大开发工具。H82的参与者自主共建仍是长期方向；目前只有既有动作选择和后台工程链，不能把推荐路线写成已完成的居民自主开发平台。
...
H82原下一项建议（H84已调整验证顺序）：由原居民自己选择一个真实需要，经过认领/委托、建设、GM自测、主AI意见评审、发布、本人体验，再自主提出后续改进、维修或放弃。复用现有恢复和发布机制，减少逐阶段人工派单；以事件唤醒、轮转探索、配方复用和有限施工并发控制计算成本。网框可成为候选，但不强制选题/认领/使用；维护者仍可主动处理公开职责内的引擎故障。保留每次主AI意见评审，不增加自动质量门禁。该实验尚未运行，不能用设计评分替代实际证据。H85已完成该接口对照；当前优先项见上方H85，不先搭建完整生活模块平台。
...
flowchart TD
    ...
    H82 -.减少人工逐阶段派发.-> H47
    H82 -.生活需要驱动建设与扩张.-> N5
    H84["H84 模型持续增强前提下五人重审<br/>自主生活与可替换接口；模型/接口对照待验，不先造完整模块平台"]
    H82 -.重审框架前提.-> H84
    H84 -.验证人工接力与接口限制.-> H47
    H84 -.保留持久世界基础.-> M20
