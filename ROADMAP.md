...
目前已有独立十人世界的实际街道与同档恢复、十个真实DeepSeek GM观察先例，以及十居民/十GM脚本传输的端到端联调；**M20尚未完成，真实Kimi＋DeepSeek的共同生活/开发闭环仍待验。** [本轮源码架构复核](docs/ARCHITECTURE.md)已追踪真实入口、规则、感知、模型、存档、美术与开发链。M20A—M20H是围绕共同运行逐步补齐的工作，不是要求全部完成后才能启动的串行关卡。先补最小启动所需部分，后续由真实问题驱动。
...
flowchart TD
    ...
    H39["H39 公共地点与首次真实采用（有界绿）<br/>本人来源知识37条 · 6名居民自愿前往 · 2次公共点休息"]
...
| M20F | 三人同档冷恢复、控制器epoch/去重与部分故障注入已有；[首条真实闭环](docs/validation/real_ten_2026-09-12/report.md)：同一世界两段有界真实运行（各20次请求，engine 0、全部自有进程0、`stop-on-idle` 结束）与暂停冷恢复逐字节一致 | 首次部分满足：有界十人+十GM、带未完任务冷恢复与受控更新续接已实测；**持续的20 agent服务、长时/容量/故障注入与H34 GL崩溃仍未验**。后续追加：长时运行、完整故障隔离/冲突注入与容量覆盖；已有未知费用继续保留，不重复核销，记录等待/主机负载/帧时。 |
...
| H39 | [公共地点与首次真实采用](docs/validation/town_places_2026-09-13.md)：扩建街区四个公共地点各有十个实测可站立点；居民只有本人读到旧市场出口的真实路牌（8米内且有视线）或亲眼看到才可选。真实运行（既有launcher、kimi-k2.6、20次请求、并发1、300秒配置上限、实际165秒）中：10名居民全部形成本人来源知识（36条读牌+1条亲眼看到），20次决策里8次自愿选公共地点（6次前往+2次在点上休息），分属6名居民；6次到达各一次回执、2次休息各一次回执、0次受阻；世界 ea96edea… → a3515687…，seq 38→95，旧38事件前缀与身份/住宅/钱物/浆果不变，三处待办前后皆空；+0.5754353元、历史不确定行6→6；另起暂停冷恢复对副本逐字节一致且无重复事件 | **有界绿（仅到此范围）**：本人来源知识→自愿出行→到达/休息这条链在**一次**真实运行中被真实采用，且可冷恢复。不等于持续20-agent服务、不等于十人全部采用（当天6人前往、4人未去）、本episode无真实商队休息区到访；公共地点不产出食物/材料/技能，拥挤与完整经济/室内仍开放（H34/H36/H38 保持开放）。 |
...
| H41 | 行程停滞离线通道：time_scale 6的脚本Godot真实物理fixture 48项、地点67项、材料278项均0失败；同一live命令stall→progress→stall保留issue id，结束后关闭，阈值0.05如实导出。生成的非空场景快照通过GM consumer 6项；提交的净化合成world-scoped fixture也通过可移植6项，显式缺失集成路径exit 1。模型调用0。 | **有界绿仅限离线导出/导入**：合成fixture不是物理或居民证据，场景fixture是脚本决定且加速；尚无生产GM消费、真实新问题发现、自主诊断或同档采用，故H36/M20H仍为部分完成。 |
