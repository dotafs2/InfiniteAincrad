...
`POST /v1/route` lets the local model classify the kind of reasoning needed
before a resident turn is sent to a provider. It returns an allowlisted route
and the model mapped to that route:

```json
{
  "state": {"action": "walk_to_known_place"},
...
The built-in mapping is `local_routine -> qwen3:8b`, `cloud_social ->
kimi-k2.6`, `cloud_story_critical -> kimi-k2.6`, and `gm_review ->
deepseek-gm`. A caller may provide all four route candidates explicitly. The
policy layer forces story-critical work to the story model, world-impacting or
multi-party work to GM review, and dialogue to the social model without first
spending a local inference call. Ordinary work is sent to LocalJev; confidence
below `0.75` falls back to `cloud_social`.

This endpoint chooses a route only. It does not call a cloud provider, change a
ledger, or mutate an InfiniteAincrad save. The game host still needs to use the
returned route through a provider multiplexer and record `router_model`, route,
confidence, selected model, and fallback reason in its own receipt.
