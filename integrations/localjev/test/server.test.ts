...
function routePost(body: unknown, headers: HeadersInit = {}): Request {
  return new Request("http://localhost/v1/route", {
    method: "POST",
    headers: { "content-type": "application/json", ...headers },
    body: JSON.stringify(body),
  });
...
describe("Jev API", () => {
  ...
  test("routes a routine task and exposes the pinned model decision", async () => {
    const app = new LocalJevApp(loadSettings(), {
      ...fakeEngine,
      async decide() {
        return {
          answers: {
            route: {
              type: "choice",
              choice: "local_routine",
              probabilities: {
                local_routine: 0.92,
                cloud_social: 0.03,
                cloud_story_critical: 0.02,
                gm_review: 0.03,
              },
              confidence: 0.7,
            },
    ...
    const response = await app.fetch(routePost({
    ...
    expect(await response.json()).toEqual({
      ...
      router_model: "localjev-0.2",
    });
  });

  test("forces GM review for irreversible or multi-party work", async () => {
    const app = new LocalJevApp(loadSettings(), {
      ...fakeEngine,
      async decide() {
        throw new Error("forced route must not invoke the local engine");
      },
    });
    const response = await app.fetch(routePost({
      state: { contract: "trade" },
      task_type: "resident_trade",
      irreversible: true,
    }));
    expect(response.status).toBe(200);
    expect((await response.json()).selected_route).toBe("gm_review");
  });
});
