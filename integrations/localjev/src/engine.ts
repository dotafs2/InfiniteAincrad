...
export class Engine implements DecisionEngine {
  ...
  private async oneGroup(
    ...
    for (let attempt = 0; attempt <= this.settings.malformedRetries; attempt += 1) {
      const ollamaNative = /:\s*11434(?:\/|$)/.test(this.settings.upstream);
      const body = ollamaNative
        ? {
      ...
      const endpoint = ollamaNative
        ? `${this.settings.upstream.replace(/\/+$/, "")}/api/chat`
      ...
      try {
        ...
        throw new BackendUnavailableError(
          `inference backend unavailable: ${error instanceof Error ? `${error.name}: ${error.message}` : "network error"}`,
        );
      }
      ...
      try {
        ...
        const message = ollamaNative
          ? payload.message
          : Array.isArray(payload.choices) && record(payload.choices[0])
            ? payload.choices[0].message
            : undefined;
        if (!record(message) || typeof message.content !== "string") {
          throw new TypeError(ollamaNative ? "message content is missing" : "choices are missing");
        }
        text = message.content;
        const usage = record(payload.usage) ? payload.usage : payload;
