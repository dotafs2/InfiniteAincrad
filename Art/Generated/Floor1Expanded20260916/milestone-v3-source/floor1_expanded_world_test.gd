...
func _run() -> void:
	...
	var requested_north_grove_trial := true
	var requested_cottage_rebake_trial := false
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with("--lod-texture-tier="):
			...
			requested_north_grove_trial = arg.trim_prefix("--north-grove-trial=") in ["on", "true", "1"]
		elif arg.begins_with("--cottage-rebake-trial="):
			requested_cottage_rebake_trial = arg.trim_prefix("--cottage-rebake-trial=") in ["on", "true", "1"]
	...
	var candidate_grove: Array = world.get("_north_grove_trees")
	check("native_nw_grove_is_bounded_default_off_or_exactly_nine_additions",
		candidate_grove.size() == (9 if requested_north_grove_trial else 0) and
		int(m["north_grove_additional_native_t2_oaks"]) == candidate_grove.size() and
		bool(m["north_grove_trial_enabled"]) == requested_north_grove_trial and
		m["north_grove_bounds_m"] == {"x": [-90, -55], "z": [-313, -273]},
		{"requested": requested_north_grove_trial, "additional": candidate_grove.size(),
			"approved_focal": native_trials.size()})
	if requested_north_grove_trial and candidate_grove.size() == 9:
		var focal_mesh: Mesh = null
		if native_trials.size() == 1:
			var focal_imported: Array = native_trials[0].find_children("*", "MeshInstance3D", true, false)
			if not focal_imported.is_empty(): focal_mesh = (focal_imported[0] as MeshInstance3D).mesh
		var old_kit: Array = world.get("_north_sector_trees")
		var grounded_group := true
		var true_source_geometry := true
		var true_mesh_material_share := focal_mesh != null
		var spatial_layout_safe := true
		var group_rows := []
		for index in range(candidate_grove.size()):
			var oak := candidate_grove[index] as Node3D
			var trunk := oak.find_child("GroveTrunkOnlyCollision", true, false) as StaticBody3D
			var imported: Array = oak.find_children("*", "MeshInstance3D", true, false)
			var mesh := (imported[0] as MeshInstance3D).mesh if not imported.is_empty() else null
			var root_query := PhysicsRayQueryParameters3D.create(
				Vector3(oak.position.x, 26.0, oak.position.z),
				Vector3(oak.position.x, -2.0, oak.position.z))
			if trunk != null: root_query.exclude = [trunk.get_rid()]
			var true_floor := space.intersect_ray(root_query)
			var floor_y := float(true_floor["position"].y) if not true_floor.is_empty() else -99.0
			var actual_tris := 0
			if mesh != null:
			...
			var min_old := INF
			for old_tree: Node3D in old_kit:
				min_old = minf(min_old, Vector2(oak.position.x, oak.position.z).distance_to(
					Vector2(old_tree.position.x, old_tree.position.z)))
			var min_group := INF
			for other_index in range(candidate_grove.size()):
				if other_index == index: continue
				var other := candidate_grove[other_index] as Node3D
				min_group = minf(min_group, Vector2(oak.position.x, oak.position.z).distance_to(
					Vector2(other.position.x, other.position.z)))
			var focal_gap := Vector2(oak.position.x, oak.position.z).distance_to(
				Vector2(-55.0, -275.0))
			grounded_group = (grounded_group and not true_floor.is_empty() and
				String(true_floor.get("collider", null).name) == "NorthwestSectorTerrainCollision" and
				absf(floor_y - oak.position.y) < 0.035)
			true_source_geometry = (true_source_geometry and imported.size() == 1 and
				actual_tris == 14783 and trunk != null and
				String(oak.get_meta("asset_path", "")).ends_with("F1_oak_smart15k_forest_trial.glb") and
				float(oak.get_meta("authored_display_height_m", 0)) >= 5.99 and
				float(oak.get_meta("authored_display_height_m", 0)) <= 6.81)
			true_mesh_material_share = (true_mesh_material_share and mesh == focal_mesh and
				mesh != null and mesh.surface_get_material(0) != null and
				mesh.surface_get_material(0) == focal_mesh.surface_get_material(0))
			spatial_layout_safe = (spatial_layout_safe and oak.position.x >= -90.0 and
				oak.position.x <= -55.0 and oak.position.z >= -313.0 and
				oak.position.z <= -273.0 and min_old > 4.0 and
				min_group > 7.5 and focal_gap > 7.5 and oak.position.x < -48.0)
			group_rows.append({"name": oak.name, "site_m": oak.position,
				"actual_collision_floor_y_m": floor_y, "actual_tris": actual_tris,
				"imported_mesh_nodes": imported.size(),
				"authored_display_height_m": oak.get_meta("authored_display_height_m", 0),
				"min_old_kit_trunk_gap_m": min_old, "min_group_trunk_gap_m": min_group,
				"focal_trunk_gap_m": focal_gap})
		check("nine_candidate_oak_roots_on_actual_collidable_triangle_floor",
			grounded_group, group_rows)
		check("nine_candidate_oak_imports_are_real_14783tris_and_trunk_only",
			true_source_geometry, group_rows)
		check("nine_candidate_oaks_share_approved_focal_mesh_and_one_material_resource",
			true_mesh_material_share,
			{"focal_mesh_present": focal_mesh != null, "nine_same_resource": true_mesh_material_share})
		check("nine_candidate_oaks_bounded_avoid_road_and_other_tree_trunks",
			spatial_layout_safe, group_rows)
		var walkable_front_gap := true
		var front_gap_rows := []
		for z in [-268.0, -274.0, -282.0, -292.0]:
			var ground_query := PhysicsRayQueryParameters3D.create(
				Vector3(-50.0, 26.0, z), Vector3(-50.0, -2.0, z))
			var floor := space.intersect_ray(ground_query)
			var floor_level := float(floor["position"].y) if not floor.is_empty() else -99.0
			var body_query := PhysicsShapeQueryParameters3D.new()
			body_query.shape = bank_capsule
			body_query.transform = Transform3D(Basis.IDENTITY, Vector3(-50.0, floor_level + 0.99, z))
			var contacts := space.intersect_shape(body_query, 8) if not floor.is_empty() else []
			walkable_front_gap = (walkable_front_gap and not floor.is_empty() and
				String(floor.get("collider", null).name) == "NorthwestSectorTerrainCollision" and
				contacts.is_empty())
			front_gap_rows.append({"world_xz": Vector2(-50.0, z),
				"actual_floor_y_m": floor_level, "capsule_contacts": contacts.size()})
		check("real_0_32_by_1_80_walker_can_use_front_nw_grove_gap",
			walkable_front_gap, front_gap_rows)
	var one_cottage: Node3D = world.get("_cottage_ab_house")
	var cottage_candidate: Node3D = world.get("_cottage_rebake_visual")
	check("one_west_market_frontage_cottage_visual_ab_selects_one_only",
		one_cottage != null and
		String(one_cottage.get("house_id")) == "01_hearth_cottage" and
		String(one_cottage.get_meta("district", "")) == "MarketFrontage" and
		one_cottage.position.distance_to(Vector3(-13.9, 0, -8.0)) < 0.025 and
		absf(one_cottage.rotation_degrees.y - 91.0) < 0.025 and
		(cottage_candidate != null) == requested_cottage_rebake_trial and
		int(m["isolated_cottage_rebake_instances"]) == (1 if requested_cottage_rebake_trial else 0) and
		bool(m["isolated_cottage_rebake_trial_enabled"]) == requested_cottage_rebake_trial,
		{"house": one_cottage.name if one_cottage != null else "none",
			"candidate": cottage_candidate.name if cottage_candidate != null else "none"})
	if one_cottage != null:
		var baseline_visual := one_cottage.get_node_or_null("MeshyHouseGeometry") as Node3D
		check("one_cottage_ab_keeps_accepted_2k_lod_baseline_reversible",
			baseline_visual != null and
			baseline_visual.visible == not requested_cottage_rebake_trial and
			String(one_cottage.get_meta("asset_path", "")).ends_with("01_hearth_cottage_lod_2k.glb") and
			world.get("exterior_houses").size() == 32,
			{"baseline_visible": baseline_visual.visible if baseline_visual != null else false,
				"baseline_asset": one_cottage.get_meta("asset_path", "")})
	if requested_cottage_rebake_trial and cottage_candidate != null:
		var actual_low := Vector3(INF, INF, INF)
		var actual_high := Vector3(-INF, -INF, -INF)
		var imported_tris := 0
		var imported_materials: Array[Material] = []
		for mesh: MeshInstance3D in cottage_candidate.find_children("*", "MeshInstance3D", true, false):
			if mesh.mesh == null: continue
			var box := mesh.mesh.get_aabb()
			for xx in [box.position.x, box.end.x]:
				for yy in [box.position.y, box.end.y]:
					for zz in [box.position.z, box.end.z]:
						var point := mesh.global_transform * Vector3(xx, yy, zz)
						actual_low = actual_low.min(point)
						actual_high = actual_high.max(point)
			for surface in range(mesh.mesh.get_surface_count()):
				var arrays := mesh.mesh.surface_get_arrays(surface)
				var indices: PackedInt32Array = arrays[Mesh.ARRAY_INDEX]
				var vertices: PackedVector3Array = arrays[Mesh.ARRAY_VERTEX]
				imported_tris += int(indices.size() / 3) if not indices.is_empty() else int(vertices.size() / 3)
				var material := mesh.mesh.surface_get_material(surface)
				if material != null and not imported_materials.has(material):
					imported_materials.append(material)
		check("one_cottage_60k2k_glb_actual_imported_60000_triangles_one_material",
			imported_tris == 60000 and int(cottage_candidate.get_meta("actual_imported_triangles", 0)) == 60000 and
			imported_materials.size() == 1 and
			String(cottage_candidate.get_meta("asset_path", "")).ends_with("01_hearth_cottage_uv_rebaked_60000_color2k.glb"),
			{"actual_imported_tris": imported_tris, "materials": imported_materials.size()})
		check("one_cottage_60k2k_true_imported_display_height_floor_and_xz_centre",
			absf(actual_low.y) < 0.15 and absf((actual_high.y - actual_low.y) - 10.0) < 0.15 and
			Vector2(0.5 * (actual_low.x + actual_high.x),
				0.5 * (actual_low.z + actual_high.z)).distance_to(
				Vector2(one_cottage.position.x, one_cottage.position.z)) < 0.12,
			{"actual_low": actual_low, "actual_high": actual_high,
				"target": one_cottage.position})
