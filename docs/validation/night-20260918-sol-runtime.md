...
life04 在不改冷却、不指定行为的情况下等到牧人自然到期。03:43:48 的只读检查点为 seq206 / elapsed 7583.63：夜间账本已有 46 次 settled、零其他状态，十个居民均至少有一次本夜新 Kimi 回合，累计费用 1.1664318 CNY。牧人选择接近水槽看守；检查点已记录真实身体位移 5.2762 米，随后仍沿物理路线继续移动。该里程碑证明第十位居民的调度和身体没有被卡死，但 life04 尚在运行，最终回合验收仍由主协调任务在正常退出后收集。

园丁、织者和药草居民在 seq166 到该检查点的保存位置未改变。只读核对没有发现永久锁定：三人均无 pending job，turn 状态均为 settled，`next_due` 分别为 8357.53、8439.68、8222.68，而源码的正常 idle cooldown 为 1800 秒。seq201 冷观察仍向他们提供了采集/休息/进食、接近其他居民和已知公共地点旅行等真实移动选项；最近模型主要自愿选择 wait、ask 或 reply。织者在更早的 seq130 已有 `resident_moved`，药草居民在 seq142 完成过需要物理到场的 `harvest_ration`。因此这是冷却加自愿非移动选择，不是饥饿、机会或空间规则把人永久锁住。若要求空闲居民持续漫步，那是新的行为规格，不应在截止前用强制移动冒充现有缺陷修复。

## 尚未完成

第二回合的 10 次请求只覆盖七位不同居民，第三回合扩大到九位；life04 的牧人自然回合最终把本夜新模型覆盖补到十人。可以表述为“十位居民都有真实新模型回合”，但不能表述为“十位居民从 seq166 后都有可核验位移”：03:43:48 检查点只有七人的保存位置发生变化，园丁、织者和药草居民仍为零位移。

现存 1800 秒空闲冷却尚无证据需要改写。后续应继续自然时间并改善对活跃居民的观察入口；不应通过强制动作、伪造时间或新增自动漫步来凑齐位移覆盖率。
