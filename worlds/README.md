...
[Permanent developer usage tags](restart-20260918-01/usage/latest.md) now retain 440,067 lifetime NPC tokens and at least 2,066,704 GM tokens across 157 recorded calls, with [complete portable history and the current GM checkpoint](restart-20260918-01/usage/manifest.json). The old GM05 usage tail and GM06 timeout remain unresolved; native GM currency is unknown. H117 audited the local usage-path migration without rewriting any call, revision, session or unknown receipt. Native sessions still require explicit new epochs, and old unknown tasks must never be reissued. Follow the [usage transfer instructions](../docs/validation/actor-usage-2026-09-18.md) when moving this world. Provider billing continuity and world-scoped developer tags are separate records.
