"""Bounded Tripo text-to-model trial for one fantasy town house, model-nav test scene (2026-09-22).
...
MANIFEST_PATH = ROOT / "Art" / "Generated" / "TripoNavHouses20260922" / "trial.json"
DEFAULT_KEY_FILE = ROOT / "secrets" / "tripo-key.txt"
TMP_DIR = ROOT / "tmp" / "tripo-nav-houses-20260922"
EXPORT_DIR = ROOT / "exports" / "tripo-nav-houses-20260922"
...
def load_manifest():
    ...
    if len(houses) != 1:
        raise SystemExit("manifest must describe exactly one house, found %d" % len(houses))
    ...
    if manifest["maximum_creations"] != 1:
        raise SystemExit("maximum_creations must stay at 1")
    return manifest
...
def command_dry_run(args):
    ...
    print(json.dumps({"command": "dry-run", "jobs": len(jobs), "planned_credits": plan["planned_credits"],
                      ...
                      "plan_path": "tmp/tripo-nav-houses-20260922/jobs-plan.json",
