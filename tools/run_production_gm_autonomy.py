...
def main(argv=None):
    ...
    for index in range(1,scope['max_cycles']+1):
        ...
        after_world=load(world)
        if any(actor not in allowed or code!=allowed[actor].get('status')
               or not controller_error_matches(after_world,actor,allowed[actor])
               or not controller_error_matches(load(before_world),actor,allowed[actor])
               for actor,code in model_errors.items()):
        ...
        policy['paths']['evidence']=str(evidence); policy['paths']['state_dir']=str(resolve(scope['gm_state_dir']))
    ...
    clean_stops={'operator_stop_file','time_limit','lifetime_model_call_limit'}
